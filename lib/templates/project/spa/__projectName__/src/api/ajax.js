...
let handleUnauthorized = () => {
    let redirect = getUrlParams(location.href, 'redirect');
    if (redirect) {
        return null;
    }
    let currentUrl = encodeURIComponent(location.href);
    let toUrl = redirect ? currentUrl : loginUrl + currentUrl;
...
axios.interceptors.response.use(
    (response = {}) => {
        ...
        if ((response.config.params && response.config.params.originData) || (response.config.data && (typeOf(response.config.data) === 'string' || typeOf(response.config.data) === 'array') && response.config.data.indexOf('originData') > -1)) return data;
